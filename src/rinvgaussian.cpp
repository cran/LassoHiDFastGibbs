...
/**
 * Code for three different methods for sampling from the inverse gaussian
 * distribution.
 *
 * 1. rrinvgauss
 *   - fastest, but prone - in rare occassions - to suffer from numerical
 *     problems when mu is close to zero.
 *
 * 2. rinvgauss_safe
 *   - slower, but provides a correction when numerical issues arise.
 *
 * 3. rinvgauss_slice
 *   - slowest, an alternative approach for if 1. and 2. don't work.
 */
...
/*
 *  Helper function for rinvgauss_safe
 */

arma::vec rinvgauss_work(arma::vec vmu, arma::vec vlambda)
{
  int n = vmu.n_rows;
  arma::vec vphi = 1/vlambda;
  arma::vec vr = arma::zeros(n);
  arma::uvec cond1 = find((vmu> 0) && (vphi> 0));
  arma::uvec cond2 = find((vmu<=0) || (vphi<=0));
  ...
  * Note: The line above should yield all vx>0, but it occasionally doesn't due to
  ...
  arma::uvec cond3 = find(vx<=0);
  if (cond3.n_elem>0) {
    vx.elem(cond3) = (1/(vy.elem(cond3) % vphi.elem(cond3)));
  }

  arma::vec vu = randu(n);
  arma::uvec cond4 = find(vu< (1.0/(1+vx)));
  arma::uvec cond5 = find(vu>=(1.0/(1+vx)));
...
arma::vec rinvgauss_safe(arma::vec vmu, arma::vec vlambda) {
  ...
  arma::vec result = sqrt(vmu % vlambda) % rinvgauss_work(vm, vl);
  return result;
}
...
/*
 * Helper functions for rinvgauss_slice
 */
...
double rinvgauss_slice_scalar(double x, double a_val, double b_val, double c_val)
{
  double TOL = 1.0E-8;
  double MAXITER = 1000;
...
arma::vec rinvgauss_slice(arma::vec vx, arma::vec vmu, arma::vec vlambda)
{
  ...
  for (int i=0; i<n; ++i) {
    ...
    double val = rinvgauss_slice_scalar(vx(i), 1.5, vb_val(i), vc_val(i));
    result(i) = val;
  }
  return result;
}
